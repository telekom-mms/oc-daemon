...
func TestLoadTemplates(t *testing.T) {
	...
	// valid template file
	oldTemplates := defaultTemplate
	defer func() {
		// cleanup after test
		defaultTemplate = oldTemplates
		LoadedTemplates = ""
	}()
	valid := "valid template"
	if err := os.WriteFile(f, []byte(valid), 0600); err != nil {
		t.Fatal(err)
	}
	if err := LoadTemplates(f); err != nil {
		t.Errorf("valid template file returned error: %s", err)
	}
	if defaultTemplate == oldTemplates {
		t.Error("load did not update templates")
	}
	if LoadedTemplates != valid {
		t.Errorf("unexpected loaded templates: %s", LoadedTemplates)
	}
}
